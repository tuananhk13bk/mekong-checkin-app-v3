...
WHERE work_order.status_id = $1
  AND rfid.enable_var = true
  AND rfid.rfid_tag_num = $2
  AND work_order.trans_arrival_datetime = $3
`

async function readValidOrder(req, res) {
  // status of order we want to read => AUTHENTICATED
  const AUTHENTICATED = 5
  const rfidTagNum = req.params.rfidTagNum
  const today = moment().format('MM-DD-YY')
  const client = await dbConnect()
  try {
    const result = await client.query(QUERY_STRING, [AUTHENTICATED, rfidTagNum, today])
